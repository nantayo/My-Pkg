...
local function fetch_chnlist()
    ...
    local file_path = rule_path .. "/chnlist"
    local gfwlist_path = rule_path .. "/gfwlist"
    local tmp_file_path = file_path .. ".tmp"
    local input_file = io.open(file_path, "r")
    local output_file = io.open(tmp_file_path, "w")
    local gfwlist_file = io.open(gfwlist_path, "a")

    for line in input_file:lines() do
        if line:find("cn.bing") then
            gfwlist_file:write(line .. "\n")
        else
